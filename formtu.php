...
<div id="led">
  <h1 style="text-align: center;">  تسجيل سائق</h1>
...
<select class="selectpicker" name="tix" required style="margin-left: 10px;">
<option value=""> الرجاء اختيار اسم المكتب </option>
  <?php
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
       ...
       echo '  
<option value="'.$row['namem'].'"> '.$row['namem'].' </option>
...
if (isset($_POST['reg_user'])) {
 ...
 // $namm= mysqli_real_escape_string($conn, $_POST['tix']);
  $namm=$_POST['tix'];
...
echo "لم تقم بلموافقة على شروط الاستخدام";
  ...
  $user_check_query = "SELECT * FROM usertu WHERE idp='$id'  or (phon='$phon') LIMIT 1";
  ...
  if (count($errors) == 0 && $nam !='admin') {
    ...
    $query = "INSERT INTO `usertu`(`id`, `name`, `namem`, `location`, `phon`, `telp`, `idp`,  `pasword`, `mony`,`stat`,`term`,`email`,`img`) VALUES (NULL, '$nam', '$namm','$loc', '$phon', '$tel', '$id', '$paswordd','0','tixu','$accpt','$email','NULL');";
