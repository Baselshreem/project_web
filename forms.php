...
<div id="b"><input type="" name="namem">اسم المحل</div> <br>
<div id="b"> نوع المحل 
<select name="tyem" id="cars">
  <option value="pay">حساب للبيع</option>
  <option value="dis">حساب للعرض</option>
...
if (isset($_POST['reg_user'])) {
 ...
 $tyem= mysqli_real_escape_string($conn, $_POST['tyem']);
...
echo "لم تقم بلموافقة على شروط الاستخدام";
  ...
  $user_check_query = "SELECT * FROM users WHERE idp='$id' OR (idm='$idm') or (phon='$phon') LIMIT 1";
  ...
  if (count($errors) == 0 && $nam !='admin') {
    ...
    $query = "INSERT INTO `users`(`id`, `name`, `namem`, `location`, `phon`, `telp`, `idp`, `idm`, `pasword`, `mony`,`stat`,`term`,`email`,`tyem`) VALUES (NULL, '$nam', '$namm','$loc', '$phon', '$tel', '$id','$idm', '$paswordd','0','sal','$accpt','$email','$tyem');";
        echo " تم التسجيل بنجاح";
     mysqli_query($conn, $query);

$queryu = "SELECT * FROM `users` where 1;";
        echo " تم التسجيل بنجاح";
