...
<a href="notf.php"><div id="b" style="text-align: center;">الاشعارات  
	...
	<a ><div id="b" style="text-align: center;height: 50px;">  الحالة  <?php echo $_SESSION['statt']; ?>
<form method="post" >
<button style="background-color: yellow;" name="s">شاغر</button>

<button style="background-color: green;" name ="rw"> طلب للموقع</button>

<button name="ra">طلب عادي</button>
</form>
...
if(isset($_POST['s'])){
    ...
    $sql = mysqli_query($conn," UPDATE usertu SET statt = 'شاغر ' WHERE id = '$id' ");
...
if(isset($_POST['rw'])){
    ...
    $sql = mysqli_query($conn," UPDATE usertu SET statt = 'طلب للموقع ' WHERE id = '$id' ");
   // $result = mysqli_query($conn, $sql);
...
if(isset($_POST['ra'])){
    ...
    $sql = mysqli_query($conn," UPDATE usertu SET statt = 'طلب عادي  ' WHERE id = '$id' ");
    //$result = mysqli_query($conn, $sql);
...
$sql = "SELECT * FROM usertu where id='$id'; ";
$result = mysqli_query($conn, $sql);
//echo $n;
